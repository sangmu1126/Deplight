...
class DashboardApiTests(unittest.TestCase):
    ...
    def test_service_detail_queries_composite_analysis_key(self):
        ...
        old_deployment, old_analysis = dashboard.deployment_table, dashboard.ai_analysis_table
        dashboard.deployment_table, dashboard.ai_analysis_table = deployment, analysis
        try:
            ...
            dashboard.deployment_table, dashboard.ai_analysis_table = old_deployment, old_analysis

    def test_complete_updates_existing_deployment(self):
        table = FakeTable({"id": "deploy-1", "status": "in_progress"})
        old_table, old_key = dashboard.deployment_table, dashboard.DASHBOARD_API_KEY
        dashboard.deployment_table, dashboard.DASHBOARD_API_KEY = table, "test-key"
        try:
            response = asyncio.run(dashboard.complete_deployment(
                {"deployment_id": "deploy-1", "deployment_url": "https://example.test"},
                x_api_key="test-key",
            ))
            self.assertTrue(response["success"])
            self.assertEqual(table.updated["Key"], {"id": "deploy-1"})
        finally:
            dashboard.deployment_table, dashboard.DASHBOARD_API_KEY = old_table, old_key
